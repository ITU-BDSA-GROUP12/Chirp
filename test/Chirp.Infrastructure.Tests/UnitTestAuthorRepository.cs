...
public class UnitTestAuthorRepository
{
    ...
    [Fact]
    public async void TestThatFollowFollowersDoesNotReturnAuthorsId()
    {
        //Arrange
        var connection = new SqliteConnection("DataSource=:memory:"); //Configuring connenction using in-memory connectionString
        connection.Open(); // Open the connection. (So EF Core doesnt close it automatically)
        ...
        Task _1 = repository.FollowAnAuthor("A email", "B name");
        Task _2 = repository.FollowAnAuthor("A email", "C name");
        Task _3 = repository.FollowAnAuthor("B email", "A name");
        Task _4 = repository.FollowAnAuthor("C email", "A name");
        await _1; await _2; await _3; await _4;

        // Assert

        List<Guid>? list_should_be_empty = await repository.GetFollowersFollower("A email");
        list_should_be_empty.Should().BeEmpty();
    }


    [Fact]
    public async void TestThatUserDoesNotGetRecommendedAuthorsTheyAlreadyFollow()
    {
        //Arrange
        var connection = new SqliteConnection("DataSource=:memory:"); //Configuring connenction using in-memory connectionString
        connection.Open(); // Open the connection. (So EF Core doesnt close it automatically)
        ...
        Task _1 = repository.FollowAnAuthor("A email", "B name");
        Task _2 = repository.FollowAnAuthor("A email", "C name");
        Task _3 = repository.FollowAnAuthor("A email", "D name");
        Task _4 = repository.FollowAnAuthor("B email", "D name");
        Task _5 = repository.FollowAnAuthor("C email", "D name");

        await _1; await _2; await _3; await _4; await _5;
